...
- 副作用1：a.以上语法在`.qmd`文档内不能正常预览该公式。b.在html输出下，引用语法`\eqref{eq2}`无法显示，且两个子方程不会自动标号（但在pdf输出下能正常显示，具体可参看节[pdf测试](https://huhuaping.github.io/xmerit/dev/articles/equation-test/formats-pdf.qmd#sec-lx-label-eqref)
  ）。
...
[`xmerit::lx.psm()`](https://huhuaping.github.io/xmerit/dev/reference/lx.psm.md)函数默认形式为嵌套结构`\begin{align}`内含`\begin{split}`。
...
[`xmerit::lx.est()`](https://huhuaping.github.io/xmerit/dev/reference/lx.est.md)函数默认形式为嵌套结构`\begin{equation}`内含`\begin{alignedat}{999}`。
...
[`xmerit::qx.psm()`](https://huhuaping.github.io/xmerit/dev/reference/qx.psm.md)函数默认形式为嵌套结构`\begin{aligned}`内含`\begin{split}`。
...
[`xmerit::qx.est()`](https://huhuaping.github.io/xmerit/dev/reference/qx.est.md)函数默认形式为嵌套结构`\begin{alignedat}{999}`内含`\begin{split}`。
