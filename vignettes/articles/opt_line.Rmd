...
`opt_line()` 用于在 **Quarto**（.qmd）中生成问卷题项的一行或多行选项，支持 **HTML** 与 **Word (docx)** 双格式。输出格式会根据当前 Quarto 渲染目标自动选择：HTML 下可输出真实勾选框或字母选项，Word 下输出适合打印填写的符号与间距。

适用场景：问卷、量表、选择题的选项行，需要同时输出网页版和 Word 打印版时。

# 基本用法

在 .qmd 中，题干单独一行，选项行用内联 R 调用 `opt_line()`，例如：

- 题干：`1. 您的性别：`
- 选项行：`` `r opt_line(c("男", "女"))` ``

函数返回一整行（或多行）的选项字符串；在 HTML 勾选框模式下会通过 `knitr::asis_output()` 输出原始 HTML，其它情况为普通字符，可直接嵌入正文。

# 两种样式

## 勾选框样式（默认）

`style = "checkbox"`（或通过 `options(questionnaire.opt_style = "checkbox")` 设为默认）：
...
- `wrap = TRUE`：按显示宽度换行。
- `max_width`：单行最大宽度（半角单位，默认 42）。中文字符计 2 单位，英文/数字计 1 单位。
- `indent_wrap`：换行后的缩进字符串（默认 3 个空格），使续行与首行选项对齐。
...
3. **需要换行时**：在 `opt_line()` 中设置 `wrap = TRUE` 并可按需调整 `max_width`、`indent_wrap`。
